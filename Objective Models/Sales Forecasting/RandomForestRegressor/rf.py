...
plt.plot(monthly_sales['Period'], monthly_sales['Value'])
# predicted values
plt.plot(predict_df['Period'], predict_df['RandomForest Prediction'])
plt.title("Customer sales Forcast using RF Model")
plt.xlabel("Date")
plt.ylabel("Sales")
plt.legend(['Actual Sales', 'Predicted Sales'])
plt.show()

lr_mse = np.sqrt(mean_squared_error(predict_df['RandomForest Prediction'], monthly_sales['Value'][-12:]))
lr_mae = mean_absolute_error(predict_df['RandomForest Prediction'], monthly_sales['Value'][-12:])
lr_r2 = r2_score(predict_df['RandomForest Prediction'], monthly_sales['Value'][-12:])

print("Randomforest Regression MSE: ", lr_mse)
print("Randomforest Regression MAE: ", lr_mae)
print("Randomforest Regression R2: ", lr_r2)


april_sales = predict_df[predict_df['Period'] == '2019-01-06']['RandomForest Prediction'].values[0]
